...
import { CopyRequest } from "ollama";
import styles from '../page.module.css';


const INITIAL: CopyRequest = {
  source: "",
  destination: "",
  // template: "",
  // system: "",
...
// {"status":"writing manifest"}
// {"status":"success"}
//import getPublicFile from ./

export default function CopyForm({ onSuccess }: { onSuccess?: (data: any) => void }) {
  const [form, setForm] = useState<CopyRequest>(INITIAL);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<any | null>(null);
    const workerRef = useRef<Worker>('' as any);
    useEffect(() => {
    workerRef.current = new Worker(new URL("../fileworker.ts", import.meta.url));
...
const handleWork = useCallback(async (x: any) => {
    workerRef.current?.postMessage(x);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    setResult(null);
    const modell = handleWork (form.source)
   console.log(modell,'modell')
    try {
      // Strip empty optional fields before sending
      const payload: Record<string, unknown> = { source: modell, destination: form.destination, stream: false };
      ...
      // /{ error: "json: cannot unmarshal object into Go struct field CopyRequest.source of type string" }
      setResult(data);
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className={styles.form}>
      ...
      {result && (
        <pre style={{ background: "#335863", padding: 8,  textShadow: "0 0 8px #00ffff" }}>
          {JSON.stringify(result, null, 2)}
        </pre>
      )}

      <button type="submit" disabled={loading} className={styles.dugme}>
        {loading ? "Submitting…" : "Copy Model"}
      </button>
    </form>
    </>
  );
