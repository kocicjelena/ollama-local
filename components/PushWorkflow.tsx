...
type Step = "blob" | "create" | "show" | "push";

const STEPS: { key: Step; label: string; description: string }[] = [
  { key: "blob", label: "1. Upload Blob", description: "Upload your model blob to the Ollama server" },
    { key: "create", label: "2. Create Model", description: "Create a new model from the uploaded blob" },
  { key: "show", label: "3. Review Model", description: "Inspect the model details before pushing" },
  { key: "push", label: "4. Push to Cloud", description: "Push your model to the Ollama registry" },
];
...
export default function PushWorkflow() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        {STEPS.map((step) => (
          <button
            ...
            style={{
              flex: 1,
              padding: "8px 4px",
              background: currentStep === step.key ? "#408080" : "#e0e0e0",
      ...
      {currentStep === "blob" && (
        <BlobsForm
          onSuccess={() => setCurrentStep("create")}
        />
      )}
       {currentStep === "create" && (
        <CreateForm
      ...
      {currentStep === "push" && (
        <>
          ...
          <div style={{ marginTop: 16 }}>
            <button onClick={() => setCurrentStep("show")} className={styles.main2}> 
              ← Back to Review
            </button>
          </div>
        </>
